...
def cleanData():
    ...
    try:
        # Delete bookings that finished 6 days ago or more
        cur.execute("""
            DELETE FROM bookings
            WHERE finish <= date('now', '-6 day');""")
        ...
        if request.method == 'POST':
            ...
            if session.get('user_id'):
                if request.form.get("fav_btn") == 'fav':  # Check if user is trying to favourite a truck
                    sql_fav = """
                                    INSERT OR REPLACE INTO users_favourites (user_id, truck_id, email_reminder)
                                    VALUES (?, ?, ?);"""
                    cur.execute(sql_fav, (session['user_id'], id_x, 1,))  # Favourite truck
                    con.commit()
...
def site():
    ...
    if site_x:
        ...
        sql_bookings = """
            ...
            AND b.truck_id = t.truck_id
            ORDER BY b.start;"""
        cur.execute(sql_bookings, (site_x[0],))
